plot4 <- function() {
    ...
    # Determining which sources were coal combustion-related is very 
    # subjective, and after much contemplation and a thorough reading of 
    # the message boards I decided to filter for this source is by
    # searching for "coal" and "comb" in the Short.Name column.
    
    # Also ran a test doing a different set of filtering and the results 
    # were VERY close.  So I concluded this method of filtering 
    # is sufficient.
    
    # First attach the SCC data to the NEI table using merge
    merged <- merge(NEI,SCC,by.x="SCC",by.y="SCC",all=FALSE)
    
    # Get a list of coalSources by looking for "coal" and "comb" in the SCC table
    coalSources <- grep("(Comb.*Coal)|(Coal.*Comb)",SCC$Short.Name, value = TRUE,ignore.case=TRUE)
    
    # subset merged data by only those observations that appear in coalSources
    coalData<-merged[merged$Short.Name %in% coalSources,]
    
    # Next we will need to group by year and do a sum
    yearlyTotal <- aggregate(coalData$Emissions, by=list(coalData$year), FUN = sum, na.rm = TRUE)
